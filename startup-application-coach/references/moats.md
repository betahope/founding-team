# Competitive advantage: moats that hold up (and optional web research)
...
## The real defensibilities (the menu)

- **Network effects.** Each new user makes the product better for existing users. The strongest moat, but usually earned at scale, not present at seed. If the product has a real network effect, this is the headline.
- **Switching costs and embedding.** The product is woven so deeply into the customer's workflow, data, and integrations that leaving is painful. This is the most achievable real moat for early B2B software.
- **Brand.** Trust and category ownership. Slow to build, rarely credible at seed, but name it if it is the long-term play.
...
You cannot build a moat on the newest technology, because it is obsolete tomorrow. Instead, wrap yourself around the customer's data and workflows, and control the chokepoints: the narrowest, highest-value points in the customer's system where value is actually created. This rewards founders who understand the microeconomics of their customer's industry deeply enough to find the structural bottleneck and own it. Customer intimacy, workflow integration, and earned trust are the moats that compound even as the underlying tech keeps shifting. For a software founder with no obvious moat, this is usually the most honest and persuasive answer: deep workflow embedding at a specific chokepoint.
...
Finding the defensible chokepoint means understanding the customer's workflow and industry economics, which a founder can research. When the founder is stuck on this question, offer:

> "Finding your moat usually means finding the chokepoint in your customer's workflow: the narrow, high-value step where you can embed so deeply that leaving hurts. Want me to research your space for where those bottlenecks and switching costs tend to be?"

If yes, and web search is available in this environment, run several targeted searches: how the target customer's core workflow operates today, where the expensive or high-friction steps are, what tools they already use and how entrenched those are (to gauge switching costs), and how incumbents in the space defend their position. Use what you find to help the founder name a specific, defensible chokepoint, then write the answer around it.

Never manufacture a moat the evidence does not support. If the honest conclusion is "the moat is early and being built," say that clearly and frame the sequencing. If web search is not available in this environment, say so and work from what the founder already knows about their customer's workflow.
